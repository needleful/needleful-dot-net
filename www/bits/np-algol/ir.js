...
function loop_while(condition, body) {
	return [IR.for_while, condition, body];
}
...
const ir_to_assembler = (ir_mod) => {
	...
	const compile_statement = (s, m, p, params) => {
		// TODO: set scoping rules
		const get_param = (id) => {
			if(id in params) {
				return params[id];
			}
			else {
				throw new Error("Unknown parameter using id `"+id+"` of type "+typeof(id));
			}
		}
		if(s in params) {
			let v = params[s];
		...
		case IR.assign: {
			let v = get_param(s[1]);
			let c = compile_statement(s[2], m, p, params);
			if (v[1] != c.type){
				throw new Error("Mismatched type between variable "+s[1]
					+" of type "+v[1]+" and expression "+s[2]+" of type "+c.type);
			}
			return {type: T.block, code:[c.code, I.lset, leb_const(v[0])]};
		} break;
		case IR.if_else: {
			let condition = compile_statement(s[1], m, p, params);
			if(condition.type != T.i32) {
				console.log(s);
				console.log(condition);
				throw new Error('Conditions should be of type i32, not provided type: '+ condition.type);
			}
			let iftrue = compile_statement(s[2], m, p, params);
			let iffalse = compile_statement(s[3], m, p, params);
		...
		case IR.ret: {
			let exp = compile_statement(s[1], m, p, params);
			if (exp.type != p.type) {
				console.log(s[1]);
				console.log(p);
		...
		case IR.loop_while: {
			...
			let body = compile_expression(s[2], m, p, params);
			if(body.type != T.body) {
				console.log(s[2]);
				console.log(s);
			...
			return {
				...
				code:[ 
					I.block, T.block, I.loop, T.block,
						condition.code, I.i32eqz, br_if, 1,
						body.code,
						br, 0,
					I.end, I.end
				]
		...
		case IR.if_then: {
			...
			return {
				type: T.block,
				code: [
					I.block, T.block, condition.code, I.i32eqz, br_if, 0,
						body.code,
					I.end
				]
			};
		...
		case IR.block: {
			...
			for (let i = 1; i < s.length; i++) {
				let c = compile_statement(s[i], m, p, params)
				if(c.type != T.block){
					console.log(s[i]);
					console.log(s);
		...
		default:
			throw new Error("Unhandled IR type: "+type);
			break;
		}
	};
