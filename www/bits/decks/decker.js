...
monad={
 ...
 count:  x=>lmn(count(x)),
 first:  x=>lis(x)?lms(x.v[0]||''):            lit(x)?monad.first(rows(x)): lion(x)?lms(x.n): count(x)?ll(x)[0]: NONE,
...
toggle_fullscreen=_=>{
 if(is_fullscreen()){
  ...
  const e=q('#decker'), o={navigationUI:'hide'}
...
q('#decker').addEventListener('mousedown'  ,e=>mouse(e,down))
q('#decker').addEventListener('mouseup'    ,e=>mouse(e,up  ))
q('#decker').addEventListener('mousemove'  ,e=>mouse(e,move))
q('#decker').addEventListener('contextmenu',e=>e.preventDefault())
q('#decker').addEventListener('touchstart' ,e=>{e.preventDefault(),touch(e,down)},{passive:false})
q('#decker').addEventListener('touchend'   ,e=>touch({targetTouches:e.changedTouches},up))
q('#decker').addEventListener('touchmove'  ,e=>touch(e,move))
q('#decker').onwheel=e=>ev.scroll=e.deltaY<0?-1:e.deltaY>0?1:0
q('#display').addEventListener("keydown", e=>{
 initaudio()
 keydown[e.key]=1
...
q('#display').addEventListener("keyup",e=>{
 ...
 if(!wid.infield&&!wid.ingrid&&uimode=='interact'&&card_is(con())){
...
q('#decker').onblur=e=>{ev.alt=0,keydown={}}
let local_clipboard=''
getclipboard=after=>{
...
q('#decker').ondragover=e=>e.preventDefault()
q('#decker').ondrop=e=>{
...
async function loadDeck(file) {
  ...
  pushstate(lmenv());
  load_deck(deck_read(await response.text()));
  const tag=decodeURI(document.URL.split('#')[1]||'');
  if(tag.length) {
    iwrite(deck,lms('card'),lms(tag));
  }
  resize();
  requestAnimationFrame(loop);
  document.title=ls(ifield(deck,'name'));
}